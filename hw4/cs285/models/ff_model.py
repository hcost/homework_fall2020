...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # ODO(Q1) compute delta_pred_normalized and next_obs_pred
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std = data_statistics['obs_mean'], data_statistics[
            'obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'], data_statistics['delta_mean'], \
                                                                      data_statistics['delta_std']

        target = torch.Tensor(normalize(next_observations - observations, delta_mean, delta_std)).to(ptu.device)
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.


        output = self(observations, actions, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)[1]
